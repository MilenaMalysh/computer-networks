...
public class Strategy {
    ...
    private DatagramMode dmode;
    private VirtualChannelMode vmode;
    ...
    public void onPlayButton(TestNet.PlayEvent event){
        ...
        if (TestNet.PlayEvent.mode==0){
            dmode = new DatagramMode(random,nodes,graph);
            play(dmode);
        }else{
            vmode = new VirtualChannelMode(random,nodes, graph);
            play(vmode);
        }
    ...
    public void onSendOneMessage(FromOneToAnother.PlayEvent event){
        counter =100;
        stopping = false;
        if (FromOneToAnother.PlayEvent.mode==0){
            dmode = new DatagramMode(random,nodes,graph);
            play(dmode);
        }else{
            vmode = new VirtualChannelMode(random,nodes, graph);
